...
class Student {
	...
	public function get_query() {
		...
		$meta_query = wpFluent()->table( 'usermeta' )->where( 'wp_capabilities', 'like', '%"student"%' )->select( 'user_id' );
	...
	public function get_meta( &$students ) {
		...
		$this->merge_post_and_meta( $students, $meta, 'user_id' );
	}
			...
			function ( $student ) use ( $schools ) {
				$student['school'] = Arr::first(
					...
					function( $school ) use ( $student ) {
						return $school->ID === $student['school'];
					},
					array()
				);
				return $student;
	...
	public function get_course_group_membership_count( &$students ) {
		...
		$query->groupBy( 'post_type, user_id' );
